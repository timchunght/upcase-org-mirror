...
describe Sale do
  ...
  it 'creates the given purchase with the given params' do
    Sale.new(purchase, purchase_params(purchase: { name: 'New Name' }))
      .complete
  ...
  it 'sets the coupon if one is present' do
    ...
    Sale.new(purchase, purchase_params(coupon_id: coupon.id))
      .complete
  ...
  it 'does not set a coupon if no coupon id is present' do
    ...
    Sale.new(purchase, purchase_params).complete
  ...
  it 'sets the user if the user is present' do
    ...
    Sale.new(purchase, purchase_params, user).complete
  ...
  it "sets the stripe id if user is present and we're using an existing card" do
    ...
    Sale.new(purchase, purchase_params(use_existing_card: 'on'), user)
      .complete
  ...
  it 'does not set the stripe id if there is no user' do
    ...
    Sale.new(purchase, purchase_params(use_existing_card: 'on'))
      .complete
  ...
  it 'does not set the stripe id if we are not using an existing card' do
    ...
    Sale.new(purchase, purchase_params(use_existing_card: 'off'), user)
      .complete
